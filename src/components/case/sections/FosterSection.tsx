...
export function FosterSection({ c, canEdit, user }: SectionProps) {
  ...
  if (!c.foster_start) return <div className="text-sm text-muted-foreground">该 Case 无寄养安排（在 ① 分区填寄养起止日期）。</div>;
  const submit = () => { if (note.trim()) { addFosterDaily(c.id, note.trim(), user.id); setNote(''); } };
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-sm"><Home className="size-4 text-muted-foreground" /> 寄养 {fmtMD(c.foster_start)} ~ {fmtMD(c.foster_end)} · {c.foster_notes}</div>
      {canEdit && (
      ...
      <ul className="space-y-1.5">
        {[...c.foster_daily].reverse().map((d, i) => (
          <li key={i} className="flex gap-3 rounded-lg bg-muted/60 px-3 py-2 text-sm">
            <span className="w-14 shrink-0 text-xs text-muted-foreground">{fmtMD(d.date)}</span>
      ...
      <TextAreaField label="寄养日志" value={c.foster_log} editable={canEdit} rows={2} onChange={(v) => updateCase(c.id, { foster_log: v }, user.id)} />
    </div>
  );
}
