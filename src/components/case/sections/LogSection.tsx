...
export function LogSection({ c, canEdit, user }: SectionProps) {
  ...
  return (
    <div className="space-y-3">
      <TextAreaField label="内部备注 notes_extra" value={c.notes_extra} editable={canEdit} rows={2} onChange={(v) => updateCase(c.id, { notes_extra: v }, user.id)} />
      {canEdit && (
      ...
      <ol className="relative ml-2 border-l border-border pl-4">
        {sorted.map((l) => (
          <li key={l.id} className="relative mb-2.5 last:mb-0">
            <span className={cn('absolute -left-[21px] top-1.5 size-2.5 rounded-full ring-2 ring-white', l.actor === '系统' ? 'bg-lilac' : 'bg-primary')} />
